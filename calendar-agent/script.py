...
def parse_input(user_input):
    ...
    try:
        ...
        if dt.year == datetime.datetime.now().year and date_time_str.lower() not in [str(datetime.datetime.now().year),
...
def check_for_duplicate(service, event_details):
    """Check if an event with the same title, date, and time exists in the calendar."""
    # Search from now until end of next year
    ...
    events_result = service.events().list(
        ...
        q=event_details['summary'],  # Search by title
        singleEvents=True,
        orderBy='startTime'
    ).execute()
    events = events_result.get('items', [])

    # Localize the new event's start time to its timezone
    ...
    for event in events:
        ...
        # If existing_dt is naive, assume UTC (API should return offset-aware, but just in case)
        if existing_dt.tzinfo is None:
            existing_dt = pytz.UTC.localize(existing_dt)

        # Compare with 1-minute tolerance
        if (event['summary'].lower() == event_details['summary'].lower() and
                abs((existing_dt - new_start_dt).total_seconds()) < 60):
            print(
                f"Debug: Duplicate found - Existing: {event['summary']} at {existing_start}, New: {event_details['summary']} at {new_start_dt}")
            return True
...
def main():
    ...
    while True:
        ...
        if action == 'exit':
            ...
            user_input = input("Enter meeting details (e.g., '03/17 12pm toronto time stonemasons 1'): ")
            try:
            ...
            try:
                ...
                if 0 <= event_index < len(events):
                    ...
                    new_input = input("Enter new details (e.g., '03/18 2pm toronto time painters 5'): ")
                    try:
