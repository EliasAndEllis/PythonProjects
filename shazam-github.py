...
def load_tracker():
    """Load the playlist tracker file or create a new one if it doesn't exist."""
    tracker_file = "playlist_tracker.json"
...
def save_tracker(tracker):
    """Save the playlist tracker to a file."""
    with open("playlist_tracker.json", "w") as f:
...
def main():
    ...
    added_songs = set(f"{title} - {artist}".lower() for title, artist in tracker["added_songs"])
    not_found_songs = set(f"{title} - {artist}".lower() for title, artist in tracker["not_found_songs"])
    ...
    for title, artist in songs:
        song_key = f"{title} - {artist}".lower()
        if song_key in added_songs or song_key in not_found_songs:
            print(f"🔁 Skipping song already processed: {title} by {artist}")
            continue
        if any(title.lower() in existing_video for existing_video in existing_videos):
            print(f"🔁 Skipping song already in playlist: {title} by {artist}")
            tracker["added_songs"].append([title, artist])
        ...
        if video_id:
            ...
            tracker["not_found_songs"].append([title, artist])
            not_found_songs.add(song_key)
            print(f"⚠️ Song not found on YouTube: '{title}' by '{artist}'. Please add it manually to the playlist if desired.")
