...
if not st.session_state.game_over:
    ...
    # Ask to play again with validation and reset option (only after first play)
    if st.session_state.play_submitted and st.session_state.games > 0:
        play_again = st.text_input("Play again? (yes/no/reset):", "").lower()
        if st.button("Submit Play Again"):
            if play_again == "yes":
                st.session_state.play_submitted = False  # Continue game with current history
            elif play_again == "no":
                st.session_state.game_over = True  # Replace exit() with flag
            elif play_again == "reset":
                # Reset all game variables
                st.session_state.user_history = []
                st.session_state.computer_history = []
                st.session_state.games = 0
                st.session_state.wins = {"user": 0, "computer": 0, "ties": 0}
                st.write("Game reset! Starting fresh...")  # Replacing print
                st.session_state.play_submitted = False  # Continue game with fresh state
            else:
                st.error("Invalid choice, please enter 'yes', 'no', or 'reset': ")  # Replacing print
